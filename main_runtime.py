...
def main():
    ...
    # Precompute 3D Key points for vectorized drawing
    ...
    keys_3d_flat = np.array(keys_3d_list, dtype=np.float32)

    # Wait for first frame to initialize camera parameters
    frame = None
    while frame is None:
        frame = cam.read()
        time.sleep(0.01)

    # 1. Camera Intrinsics (Est) precomputed
    h, w = frame.shape[:2]
                ...
                for i, mid in enumerate(ids.flatten()):
                    if mid == 0: # Assuming Marker 0 is the anchor
                        ...
                        if success:
                            ...
                            pts_2d, _ = cv2.projectPoints(keys_3d_flat, rvec, tvec, K, D)
                            pts_2d = np.int32(pts_2d).reshape(CONFIG['NUM_KEYS'], 4, 2)
                            cv2.polylines(vis_frame, list(pts_2d), True, (255, 255, 0), 1)
