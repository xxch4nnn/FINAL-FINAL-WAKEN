...
class JSONScaler:
    def __init__(self, data):
        self.mean = np.array(data.get('mean', [0]))
        self.scale = np.array(data.get('scale', [1]))
    ...
    def _load_artifacts(self):
        try:
            ...
            if m_path.exists() and s_path.exists() and f_path.exists():
                ...
                self.scaler = JSONScaler(json.loads(s_path.read_text()))
                self.selected_features = json.loads(f_path.read_text())
                self.has_model = True
