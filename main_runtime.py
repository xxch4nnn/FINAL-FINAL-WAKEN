...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        # Velocity
        # ⚡ Bolt: Use math.hypot for scalar vector magnitudes (faster than np.linalg.norm in tight loops)
        ...
        # Distances
        # ⚡ Bolt: Use math.dist for small vector distances
        ...
        # ⚡ Bolt: Use list comprehensions and math.hypot for rolling stats
        recent_speeds = [math.hypot(v[0], v[1]) for v in tip_v[start_idx:]]
        recent_accs = [math.hypot(a[0], a[1]) for a in tip_a[start_idx:]]

        n_sp = len(recent_speeds)
        avg_sp = sum(recent_speeds) / n_sp if n_sp else 0.0

        feat['avg_speed'] = avg_sp
        feat['avg_acc_mag'] = sum(recent_accs) / len(recent_accs) if recent_accs else 0.0
        feat['rolling_var_speed'] = sum((x - avg_sp)**2 for x in recent_speeds) / n_sp if n_sp else 0.0
        ...
        for lag in [2, 4, 6]:
            ...
            if idx >= 0:
                # ⚡ Bolt: Use math.hypot for historical velocity magnitude
