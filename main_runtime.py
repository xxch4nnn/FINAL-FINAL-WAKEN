...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        # ⚡ Bolt: Use math.hypot for ~6x faster 2D vector magnitude than np.linalg.norm
        ...
        # Distances
        # ⚡ Bolt: Use math.dist for faster distance calculation
        ...
        # ⚡ Bolt: Use list comprehensions and native math for faster aggregations on small lists
        recent_speeds = [math.hypot(v[0], v[1]) for v in tip_v[start_idx:]]
        recent_accs = [math.hypot(a[0], a[1]) for a in tip_a[start_idx:]]
        
        l_speeds = len(recent_speeds)
        l_accs = len(recent_accs)
        feat['avg_speed'] = sum(recent_speeds) / l_speeds if l_speeds > 0 else 0.0
        feat['avg_acc_mag'] = sum(recent_accs) / l_accs if l_accs > 0 else 0.0
        feat['rolling_var_speed'] = sum((x - feat['avg_speed'])**2 for x in recent_speeds) / l_speeds if l_speeds > 0 else 0.0
