...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        # ⚡ Bolt: Use native math for small arrays to avoid NumPy dispatch latency
        n_speeds = len(recent_speeds)
        n_accs = len(recent_accs)
        avg_s = sum(recent_speeds) / n_speeds if n_speeds > 0 else 0.0
        feat['avg_speed'] = avg_s
        feat['avg_acc_mag'] = sum(recent_accs) / n_accs if n_accs > 0 else 0.0
        feat['rolling_var_speed'] = sum((x - avg_s)**2 for x in recent_speeds) / n_speeds if n_speeds > 0 else 0.0
