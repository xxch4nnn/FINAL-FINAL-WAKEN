...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        # ⚡ Bolt: Use math.hypot and math.dist instead of np.linalg.norm for small 2D vectors
        # Velocity
        ...
        # ⚡ Bolt: Use native Python lists and math for aggregation to avoid NumPy dispatch latency
        ...
        n_speeds = len(recent_speeds)
        n_accs = len(recent_accs)

        avg_speed = sum(recent_speeds) / n_speeds if n_speeds > 0 else 0.0
        avg_acc = sum(recent_accs) / n_accs if n_accs > 0 else 0.0

        feat['avg_speed'] = avg_speed
        feat['avg_acc_mag'] = avg_acc
        feat['rolling_var_speed'] = sum((x - avg_speed)**2 for x in recent_speeds) / n_speeds if n_speeds > 0 else 0.0
