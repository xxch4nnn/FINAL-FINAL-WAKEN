...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        # ⚡ Bolt: Use native math.hypot instead of np.linalg.norm for small vectors to reduce dispatch latency.
        ...
        # Distances
        # ⚡ Bolt: Use native math.dist for 2D coordinate distances instead of np.linalg.norm.
        ...
        # ⚡ Bolt: Use native python list comprehensions and math for small arrays to avoid NumPy overhead.
        recent_speeds = [math.hypot(v[0], v[1]) for v in tip_v[start_idx:]]
        recent_accs = [math.hypot(a[0], a[1]) for a in tip_a[start_idx:]]
        
        feat['avg_speed'] = sum(recent_speeds) / len(recent_speeds)
        feat['avg_acc_mag'] = sum(recent_accs) / len(recent_accs)
        avg_speed = feat['avg_speed']
        feat['rolling_var_speed'] = sum((x - avg_speed)**2 for x in recent_speeds) / len(recent_speeds)
