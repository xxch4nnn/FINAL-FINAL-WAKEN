...
import cv2
import numpy as np
import math # ⚡ Bolt: Use math.hypot for speed
...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        feat['finger_speed'] = math.hypot(t_v[0], t_v[1]) # ⚡ Bolt: Fast scalar math
        
        feat['wrist_vel_x'] = w_v[0]
        feat['wrist_vel_y'] = w_v[1]
        feat['wrist_speed'] = math.hypot(w_v[0], w_v[1]) # ⚡ Bolt: Fast scalar math
        
        # Acceleration
        feat['finger_acc_x'] = t_a[0]
        feat['finger_acc_y'] = t_a[1]
        feat['finger_acc_mag'] = math.hypot(t_a[0], t_a[1]) # ⚡ Bolt: Fast scalar math
        ...
        # Distances
        feat['dist_wrist'] = math.hypot(t_p[0] - w_p[0], t_p[1] - w_p[1]) # ⚡ Bolt: Fast scalar math
        feat['dist_palm'] = math.hypot(t_p[0] - palm_hist[curr, 0], t_p[1] - palm_hist[curr, 1]) # ⚡ Bolt: Fast scalar math
        feat['posture_dist'] = math.hypot(t_p[0] - dip_hist[curr, 0], t_p[1] - dip_hist[curr, 1]) # ⚡ Bolt: Fast scalar math
        ...
        recent_speeds = [math.hypot(v[0], v[1]) for v in tip_v[start_idx:]] # ⚡ Bolt: Fast list comp
        recent_accs = [math.hypot(a[0], a[1]) for a in tip_a[start_idx:]] # ⚡ Bolt: Fast list comp
        
        feat['avg_speed'] = sum(recent_speeds) / len(recent_speeds) if recent_speeds else 0.0 # ⚡ Bolt: Fast native mean
        feat['avg_acc_mag'] = sum(recent_accs) / len(recent_accs) if recent_accs else 0.0 # ⚡ Bolt: Fast native mean
        ...
        for lag in [2, 4, 6]:
            ...
            if idx >= 0:
                feat[f'lag_speed_{lag}'] = math.hypot(tip_v[idx][0], tip_v[idx][1]) # ⚡ Bolt: Fast scalar math
            else:
