...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        # Velocity
        # ⚡ Bolt: Use math.hypot and native math instead of np.linalg.norm for small vectors
        ...
        avg_speed = sum(recent_speeds) / len(recent_speeds) if recent_speeds else 0.0
        feat['avg_speed'] = avg_speed
        feat['avg_acc_mag'] = sum(recent_accs) / len(recent_accs) if recent_accs else 0.0
        feat['rolling_var_speed'] = sum((x - avg_speed) ** 2 for x in recent_speeds) / len(recent_speeds) if recent_speeds else 0.0
        ...
        for lag in [2, 4, 6]:
            ...
            if idx >= 0:
                v_lag = tip_v[idx]
                feat[f'lag_speed_{lag}'] = math.hypot(v_lag[0], v_lag[1])
            else:
