...
def main():
    ...
    # Pre-compute Virtual Keys 3D Geometry for vectorized projectPoints
    ...
    keys_3d_np = np.array(keys_3d, dtype=np.float32).reshape(-1, 3)
    ...
    try:
        while running:
            ...
            # Camera Intrinsics (Compute dynamically once based on actual frame shape)
            if K is None:
                h, w = frame.shape[:2]
                ...
                for i, mid in enumerate(ids.flatten()):
                    if mid == 0: # Assuming Marker 0 is the anchor
                        ...
                        if success:
                            ...
                            pts_2d, _ = cv2.projectPoints(keys_3d_np, rvec, tvec, K, D)
