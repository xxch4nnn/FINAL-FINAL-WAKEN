...
class LiveFeatureExtractor:
    ...
    def extract(self):
        ...
        # ⚡ Bolt: replace np.linalg.norm with faster native math.hypot
        ...
        # Distances
        feat['dist_wrist'] = math.hypot(t_p[0] - w_p[0], t_p[1] - w_p[1])
        feat['dist_palm'] = math.hypot(t_p[0] - palm_hist[curr, 0], t_p[1] - palm_hist[curr, 1])
        feat['posture_dist'] = math.hypot(t_p[0] - dip_hist[curr, 0], t_p[1] - dip_hist[curr, 1])
        ...
        recent_accs = [math.hypot(a[0], a[1]) for a in tip_a[start_idx:]]
        
        rs_len = len(recent_speeds)
        rs_mean = sum(recent_speeds) / rs_len if rs_len > 0 else 0.0
        feat['avg_speed'] = rs_mean

        ra_len = len(recent_accs)
        feat['avg_acc_mag'] = sum(recent_accs) / ra_len if ra_len > 0 else 0.0

        feat['rolling_var_speed'] = sum((s - rs_mean)**2 for s in recent_speeds) / rs_len if rs_len > 0 else 0.0
