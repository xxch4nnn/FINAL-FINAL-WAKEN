    ...
    def _load_artifacts(self):
        try:
            ...
            if m_path.exists() and s_path.exists() and f_path.exists():
                ...
                self.scaler = JSONScaler()
                self.scaler.mean_ = np.array(scaler_data['mean_']) if scaler_data.get('mean_') is not None else None
                self.scaler.scale_ = np.array(scaler_data['scale_']) if scaler_data.get('scale_') is not None else None
