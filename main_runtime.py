...
def main():
    ...
    h_cam = CONFIG['HEIGHT']
    f_cam = w_cam
    K = np.array([[f_cam, 0, w_cam / 2], [0, f_cam, h_cam / 2], [0, 0, 1]], dtype=np.float32)
    D = np.zeros((4, 1), dtype=np.float32)

    # Precompute Virtual Keys 3D coordinates (Vectorized)
    ...
    try:
        while running:
            ...
            # 1. Camera Intrinsics (Precomputed outside loop to avoid redundant allocation)
            h, w = frame.shape[:2]
                ...
                for i, mid in enumerate(ids.flatten()):
                    if mid == 0: # Assuming Marker 0 is the anchor
                        ...
                        if success:
                            ...
                            # Draw Virtual Keys (Vectorized projection and polyline rendering)
