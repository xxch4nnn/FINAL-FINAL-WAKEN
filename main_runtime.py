...
def main():
    ...
    for k in range(CONFIG['NUM_KEYS']):
        ...
        virtual_keys_3d.extend([
            [k_x, -0.05, 0], [k_x + CONFIG['KEY_WIDTH'], -0.05, 0],
            [k_x + CONFIG['KEY_WIDTH'], 0.15, 0], [k_x, 0.15, 0]
        ])
    virtual_keys_3d = np.array(virtual_keys_3d, dtype=np.float32)
                ...
                for i, mid in enumerate(ids.flatten()):
                    if mid == 0: # Assuming Marker 0 is the anchor
                        ...
                        if success:
                            ...
                            # Draw Virtual Keys using batched OpenCV operations
                            pts_2d, _ = cv2.projectPoints(virtual_keys_3d, rvec, tvec, K, D)
