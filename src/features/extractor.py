...
class HandFeatureExtractor:
    ...
    def _to_pixel(self, landmark):
        ...
        # Optimization: use native Python tuples instead of np.array to avoid instantiation overhead per frame
        return (
    ...
    def _get_euclidean(self, p1, p2):
        ...
        # Optimization: math.hypot is significantly faster than np.sqrt for scalar math
    ...
    def process_live(self, landmarks, distance_cm=115.0):
        ...
        if len(self.history) >= 2:
            # Optimization: for small lists, native sum()/len() is faster than the overhead of invoking np.mean()
