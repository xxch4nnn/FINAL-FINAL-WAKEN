...
class HandFeatureExtractor:
    ...
    def _to_pixel(self, landmark):
        ...
        # ⚡ Bolt Optimization: Use native tuples instead of np.array to avoid instantiation overhead in per-frame hot loop
        return (
    ...
    def _get_euclidean(self, p1, p2):
        ...
        dx = int(p1[0] - p2[0])
        dy = int(p1[1] - p2[1])
        # ⚡ Bolt Optimization: Use native math.hypot for faster euclidean distance calculation
    ...
    def process_live(self, landmarks, distance_cm=115.0):
        ...
        if len(self.history) >= 2:
            ...
            # ⚡ Bolt Optimization: Use native sum()/len() instead of np.mean() for small lists to reduce call overhead
