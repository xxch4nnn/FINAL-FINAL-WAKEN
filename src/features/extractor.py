...
class HandFeatureExtractor:
    ...
    def _get_euclidean(self, p1, p2):
        ...
        dx = int(p1[0] - p2[0])
        dy = int(p1[1] - p2[1])
        return math.hypot(dx, dy)  # ⚡ Bolt: Use math.hypot for faster scalar math
    ...
    def process_live(self, landmarks, distance_cm=115.0):
        ...
        if len(self.history) >= 2:
            ...
            feats['velocity_disp'] = sum(disps) / len(disps)  # ⚡ Bolt: native math avoids NumPy dispatch overhead
