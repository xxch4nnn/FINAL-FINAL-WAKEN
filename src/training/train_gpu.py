...
def train():
    ...
    # Save Model (Secure JSON)
    clf.save_model(MODEL_PATH)
    print(f"Model saved to {MODEL_PATH}")

    # Export Identity Scaler for Pipeline Consistency
    scaler_path = MODEL_DIR / "scaler.json"
    identity_scaler = {
        "mean_": [0.0] * len(TARGET_COLS),
        "scale_": [1.0] * len(TARGET_COLS)
    }
    with open(scaler_path, 'w') as f:
        json.dump(identity_scaler, f)
    print(f"Identity Scaler saved to {scaler_path}")
