...
class VisionEngine:
    ...
    def _load_model(self, path):
        p = Path(path)
        if p.suffix == '.pkl':
            raise ValueError("Insecure deserialization vulnerability: loading models via pickle (.pkl) is strictly prohibited. Please use native XGBoost JSON format instead.")
        if not p.exists():
